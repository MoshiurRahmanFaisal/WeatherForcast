...
for latlong, district, thana, code in zip(latlong_list, district_list, thana_list, thana_code_list):
    # print(i,j)
    url = 'http://api.weatherapi.com/v1/history.json?key=e0f350bfcc294b56a4242130221212&q='+latlong+'&dt='+str(yesterday)+'&end_dt='+str(yesterday)    
...
df_weather.drop(['location.region','location.country','location.localtime_epoch','location.localtime',
                ...
                'forecast.forecastday.astro.sunset'], axis=1,inplace=True)

#drop tables
df_weather.drop(df_weather.iloc[:, 3:9], inplace=True, axis=1)
...
df_weather.rename(columns = {'forecast.forecastday.hour.will_it_rain':'hourly_rain'}, inplace = True)
...
sqlTxt = '''INSERT INTO BI_PROD.weather_hourly_history (district_name, thana_name, thana_code, location, latitude, longitute, hourly_temperature_c, 
       hourly_is_day, hourly_condition, hourly_wind_speed_kph, hourly_wind_degree, hourly_wind_direction, hourly_preassure_in,
       total_precipitation_mm, total_precipitation_in, hourly_cloud, hourly_feelslike_c, hourly_rain, hourly_chance_of_rain, 
       event_date, event_time, hourly_humidity_c) 
