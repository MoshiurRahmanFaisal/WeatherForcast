...
#get data from api 
for latlong, district, thana, code in zip(latlong_list, district_list, thana_list, thana_code_list):
    # print(i,j)
    url = 'http://api.weatherapi.com/v1/forecast.json?key=e0f350bfcc294b56a4242130221212&q='+latlong+'&days=3&aqi=no&alerts=no&hour=1'   
...
#drop hour tables
df_weather.drop(df_weather.iloc[:, 5:33], inplace=True, axis=1)

#drop unnecessary tables
df_weather.drop(['location.region','location.country','forecast.forecastday.date_epoch',
                 'forecast.forecastday.day.maxtemp_f','forecast.forecastday.day.mintemp_f',
                ...
                'forecast.forecastday.astro.moon_phase','forecast.forecastday.day.avgvis_miles',
                'forecast.forecastday.day.totalsnow_cm','forecast.forecastday.day.daily_will_it_rain',
                 'forecast.forecastday.day.daily_chance_of_rain','forecast.forecastday.day.daily_will_it_snow',
                 'forecast.forecastday.day.daily_chance_of_snow'], axis=1,inplace=True)
...
#drop hour tables
df_weather.drop(df_weather.iloc[:, 12:50], inplace=True, axis=1)
...
DelsqlTxt = '''DELETE FROM BI_PROD.weather_daily_prediction'''
cur.execute(DelsqlTxt)
conn.commit()

#insert new data
dataInsertionTuples = [tuple(x) for x in df_weather.values]
sqlTxt = '''INSERT INTO BI_PROD.weather_daily_prediction (district_name, thana_name, thana_code, location, latitude, longitute, event_date,
